...
def reset_pose(source: str, destination: str):
    ...
    logger.info(f'Loading {os.path.basename(source)}')
...
def smooth_stiching(positions: Tensor, rotations: Tensor, prev_anim: dict, nframes: int = 30) -> Tuple[Tensor, Tensor]:
    ...
    interpolated_positions = positions[0:nframes] * (1 - 1.0 / nframes * (
        nframes - np.arange(nframes))[:, np.newaxis, np.newaxis])
    interpolated_prev_positions = prev_positions[-1] * (
        1.0 / nframes * (nframes - np.arange(nframes))[:, np.newaxis, np.newaxis])
    positions[0:nframes] = interpolated_positions + interpolated_prev_positions

    interpolated_rotations = rotations[0:nframes] * (1 - 1.0 / nframes * (
        nframes - np.arange(nframes))[:, np.newaxis, np.newaxis])
    interpolated_prev_rotations = prev_rotations[-1] * (
        1.0 / nframes * (nframes - np.arange(nframes))[:, np.newaxis, np.newaxis])
